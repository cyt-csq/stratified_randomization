...
run_single_trial <- function(
  ...
  # Step 5: 模拟二分类结局（两套独立结局）
  outcomes_strat <- numeric(total_enrolled)
  outcomes_unstrat <- numeric(total_enrolled)
  
  for (i in 1:total_enrolled) {
    # MEAN_MATRIX 现在应为概率矩阵（每层每组的事件概率，取值0-1）
    p_strat <- MEAN_MATRIX[enrolled_strata[i], enrolled_trt_strat[i]]
    outcomes_strat[i] <- rbinom(1, 1, prob = p_strat)
    
    p_unstrat <- MEAN_MATRIX[enrolled_strata[i], enrolled_trt_unstrat[i]]
    outcomes_unstrat[i] <- rbinom(1, 1, prob = p_unstrat)
  }
  ...
  # 构建2x2列联表
  unstrat_table <- matrix(c(
    sum(outcomes_trt1_unstrat == 1), sum(outcomes_trt1_unstrat == 0),
    sum(outcomes_trt2_unstrat == 1), sum(outcomes_trt2_unstrat == 0)
  ), nrow = 2, byrow = TRUE)
  
  if (n1_unstrat > 0 & n2_unstrat > 0) {
    fisher_res <- fisher.test(unstrat_table)
    p_unstrat <- fisher_res$p.value
    var_unstrat <- NA
    se_unstrat <- NA
  ...
  # 构建三维数组用于CMH检验
  cmh_array <- array(0, dim = c(2, 2, STRATA_LEVELS))
  for (k in 1:STRATA_LEVELS) {
    idx_k <- which(enrolled_strata == k)
    trt_k <- enrolled_trt_strat[idx_k]
    out_k <- outcomes_strat[idx_k]
    # 2x2表：行=trt, 列=outcome
    cmh_array[1, 1, k] <- sum(trt_k == 1 & out_k == 1)
    cmh_array[1, 2, k] <- sum(trt_k == 1 & out_k == 0)
    cmh_array[2, 1, k] <- sum(trt_k == 2 & out_k == 1)
    cmh_array[2, 2, k] <- sum(trt_k == 2 & out_k == 0)
  }
  
  if (all(dim(cmh_array) == c(2, 2, STRATA_LEVELS)) && n1_strat > 0 && n2_strat > 0) {
    cmh_res <- mantelhaen.test(cmh_array)
    p_strat <- cmh_res$p.value
    md_strat <- NA # CMH检验不直接给出均值差
    se_strat <- NA
    var_strat <- NA
  } else {
    p_strat <- NA
  ...
  return(list(
    summary = data.frame(
      ...
      between_sd_trt1 = NA,
      between_sd_trt2 = NA,
      within_pooled_sd_trt1 = NA,
      within_pooled_sd_trt2 = NA,
